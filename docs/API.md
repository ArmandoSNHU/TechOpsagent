...
Integration routes return 409 for a missing configured service, 502 for sanitized upstream failures, and 404 for an empty Loki window. Remote targets come only from config/integrations.json. GitHub reads one page of up to 20 items and excludes pull requests. Tokens are read from the process environment and never returned.
