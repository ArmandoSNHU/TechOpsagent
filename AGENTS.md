...
Preferred Windows entry point: setup.ps1. Use -Check for offline diagnostics, -Check -Live for localhost checks, -Configure -Service name for private edits, and -Run for foreground startup. Health build_id detects stale source; configuration edits still require restart. Python 3.12+ is required for this setup workflow.
